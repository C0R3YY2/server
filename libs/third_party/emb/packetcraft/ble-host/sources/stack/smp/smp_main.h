...
typedef struct
{
  bool            lescEnabled;                    /* true when LE Secure Connection pairing in use */
  uint8_t         authType;                       /* Type of authentication (Just Works, Numeric Comparison, Passkey, or OOB) */
  bool            kpNotify;                       /* true when Keypress notification in use */
  uint8_t         pkPos;                          /* Current passkey bit position */
  bool            display;                        /* Passkey display setting */
...
typedef struct
{
  ...
  bool            initiator;                      /* true if device is initiator */
  bool            secReq;                         /* true if security request sent or received */
  ...
  bool            keyReady;                       /* Encryption key is ready */
...
typedef bool smpProcPairing_t(smpCcb_t *pCcb, uint8_t *pOob, bool *pDisplay);

/* SMP process pairing function */
typedef void smpProcAuthReq_t(smpCcb_t *pCcb, uint8_t oob, bool display);
...
typedef struct
{
  ...
  bool              lescSupported;        /* true if LE Secure Connections is supported */
} smpCb_t;
...
bool smpStateIdle(smpCcb_t *pCcb);
...
bool smpProcPairing(smpCcb_t *pCcb, uint8_t *pOob, bool *pDisplay);
void smpAuthReq(smpCcb_t *pCcb, uint8_t oob, bool display);
...
bool smpSendKey(smpCcb_t *pCcb, uint8_t keyDist);
bool smpProcRcvKey(smpCcb_t *pCcb, dmSecKeyIndEvt_t *pKeyInd, uint8_t *pBuf, uint8_t keyDist);
