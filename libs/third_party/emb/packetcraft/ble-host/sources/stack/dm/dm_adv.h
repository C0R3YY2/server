...
enum
{
  ...
  DM_ADV_SYNC_PER_MSG_API_EVENT_NOTIFCATION_ENABLE,
};
...
/* Data structure for DM_ADV_SYNC_PER_MSG_API_EVENT_NOTIFCATION_ENABLE */
typedef struct
{
  wsfMsgHdr_t             hdr;
  uint8_t                 enable;
} dmAdvPerApiSetEventNotificationEnable_t;
...
/* Data structure for DM_ADV_PER_MSG_API_SET_SE_DATA */
typedef struct
{
  wsfMsgHdr_t             hdr;
  uint8_t                 advHandle;
  uint8_t                 numParams;
  hciSubevtParam_t        *pParams;
} dmAdvPerApiSetSeData_t;
...
typedef union
{
  ...
  dmAdvPerApiSetSeData_t                  apiPerSetSubEvtData;
  dmAdvPerApiStart_t                      apiPerStart;
  dmAdvPerApiStop_t                       apiPerStop;
  dmAdvPerApiSetEventNotificationEnable_t apiPerSetEventNotification;
  secAes_t                                aes;
} dmAdvMsg_t;
...
void dmPerAdvActSyncSetEventNotificationEnable(dmAdvMsg_t *pMsg);
...
/* periodic adv with response action functions */
void dmPerAdvWithRspActConfig(dmAdvMsg_t *pMsg);

/* periodic adv with response component inteface */
void dmPerAdvWithRspMsgHandler(wsfMsgHdr_t *pMsg);
void dmPerAdvWithRspHciHandler(hciEvt_t *pEvent);
void dmPerAdvWithRspReset(void);
