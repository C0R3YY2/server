...
#define DM_CONN_ACT_ID(action)              ((action) & 0x0FU)
...
dmConnId_t dmConnOpenAccept(uint8_t clientId, uint8_t initPhys, uint8_t advHandle, uint8_t subevent,
                            uint8_t advType, uint16_t duration, uint8_t maxEaEvents, uint8_t addrType,
...
/* pawr master action functions */
void dmExtPawrConnSmActOpen(dmConnCcb_t *pCcb, dmConnMsg_t *pMsg);
