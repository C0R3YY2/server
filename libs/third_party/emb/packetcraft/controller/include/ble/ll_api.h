...
// Include EM Bleu specific link layer run-time configuration parameters
...
typedef struct
{
  ...
  uint8_t   csOptTFcsTimesSup;      /*!< Optional T_FCS times supported. */
  ...
  /* EM Bleu specific LL Run-Time Configuration. */
  EmbSpecLlRtCfg_t embSpecLlRtCfg;
} LlRtCfg_t;
...
#define LL_FEAT_ALL_MASK                            (UINT64_C(0x00001FFFFFFFFFFF))  /*!< All feature mask, need to be updated when new features are added. */
...
typedef uint8_t LlIsoDataPathDir_t;
...
typedef uint8_t LlIsoDataPath_t;
...
#if (EM_DERIVED_STACK_CODE)
  /* EMSRC Receiver Test command permits custom SyncWords. */
  uint32_t syncWord;             /*!< Sync Word or Access Address for test packets. */
#endif /* END - EM_DERIVED_STACK_CODE*/
...
typedef uint8_t LlIsoPldType_t;
...
#if LHCI_ENABLE_VS
/*! \brief      VS CS test channel sequence generation. */
typedef struct
{
  uint8_t  chanSelType;            /*!< Channel selection type. */

  /* --- Time parameters used for calculation of number of steps --- */
  uint8_t  syncPacket;             /*!< Sync packet. */
  uint8_t  frequencyMeasurement;   /*!< Frequency measurement. Time in us. */
  uint8_t  guardTime;              /*!< Guard time. Time in us. */
  uint8_t  timeToHop;              /*!< Time to hop. Time in us. */
  uint8_t  rampDownTime;           /*!< Ramp down time. Time in us. */
  uint8_t  interludePeriod1;       /*!< Interlude period 1. Time in us. */
  uint8_t  interludePeriod2;       /*!< Interlude period 2. Time in us. */
  uint8_t  phaseMeasurementTime;   /*!< Phase measurement time. Time in us. */
  uint8_t  numAntennaPaths;        /*!< Number of antenna paths. */
  uint8_t  antennaSwitchTime;      /*!< Antenna switch time. Time in us. */

  /* --- Channel Selection Algorithm #3C parameters --- */
  uint8_t  shape;                  /*!< Shape of the sequence. LL_CS_SHAPE_HAT or LL_CS_SHAPE_X */
  uint8_t  channelJump;            /*!< Channel jump. */
  uint8_t  numRepetitions;         /*!< Number of repetitions. */
  uint8_t  chanMap[LL_CS_CHAN_MAP_SIZE];
                                   /*!< Channel map. */

  /* --- Mode parameters --- */
  uint8_t  mode0Duration;          /*!< Mode 0 duration. */
  uint8_t  mainModeDuration;       /*!< Mode 2 duration. */
  uint8_t  subModeDuration;        /*!< Mode 3 duration. */
  uint8_t  minMainModeSteps;       /*!< Minimum main mode steps. */
  uint8_t  maxMainModeSteps;       /*!< Maximum main mode steps. */
  uint8_t  phy;                    /*!< PHY. */
  uint8_t  rttType;                /*!< RTT Type. */
  uint16_t eventDuration;          /*!< Event duration. */
  uint8_t  mode0Steps;             /*!< Mode 0 steps. */
  uint8_t  mainModeRepetition;     /*!< Main mode repetition. */

  /* --- Entropy parameters --- */
  uint8_t  entropyInput[LL_KEY_LEN];
                                   /*!< Entropy input (CS_IV). */
  uint8_t  nonce[LL_KEY_LEN/2];
                                   /*!< Nonce (CS_IN). */
  uint8_t  personalString[LL_KEY_LEN];
                                   /*!< Personalization string (CS_PV). */
} LlVsCsChanSelParams_t;
#endif
...
enum
{
  ...
  LL_ADV_COMPLETE_IND,          /*!< Advertisement complete (vendor specific)*/
#endif //USE_EMB_EXTENSIONS_LL
};

#if(USE_EMB_EXTENSIONS_LL)
/*! \brief      Advertising complete indication.
 *
 * This extension allows the application to be aware when advertisements are
 * happening.
 */
typedef struct
{
...
typedef struct
{
  ...
  LlRole_t      role:8;         /*!< Role of this device. */
  uint8_t       addrType;       /*!< Address type. */
...
#endif /* INIT_FEAT_CHANNEL_SOUNDING */
#if(USE_EMB_EXTENSIONS_LL)
  LlAdvCompleteInd_t        advCompleteInd;         /*!< Advertisement complete event */
#endif //USE_EMB_EXTENSIONS_LL
} LlEvt_t;
...
#define LL_CS_MAX_CHAN_MAP_REP        3   /*!< Max number of channel map repetitions. */
/*! \} */    /* LL_API_CS */
 ...
 *  \return     Status error code.
 *
 *  \warning    The scheduler must run within 625us of calling this API.
 *              Operations that take too long or block the CPU immediately after
 *              calling this will cause it to silently fail.
 *
...
/*************************************************************************************************/
/*!
 *  \brief     CS Generate Channel Sequence
 *
 *  \param     pCsTestCfg        CS channel sequence parameters.
 *
 *  \return    Status error code.
 */
/*************************************************************************************************/
uint8_t LlCsGenerateChannelSequence(LlVsCsChanSelParams_t *pChanSeqParams);
