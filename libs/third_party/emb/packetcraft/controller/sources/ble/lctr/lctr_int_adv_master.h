...
typedef struct
{
  ...
  bool        enable;                 /*!< Enable advertising filtering. */
  bool        addToFiltTbl;           /*!< true if duplicate is not found and is to be added to the table. */
...
typedef struct
{
  ...
  union
  {
    ...
    struct
    {
      ...
      bool          connBodLoaded;      /*!< Connection BOD loaded flag. */
  ...
  BbBleMstAdvEvent_t bleData;           /*!< BLE BB operation data. */
...
bool lctrMstDiscoverAdvPktHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf);
void lctrMstDiscoverAdvPktPostProcessHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf);
bool lctrMstScanReqTxCompHandler(BbOpDesc_t *pOp, const uint8_t *pAdvBuf);
bool lctrMstScanRspRxCompHandler(BbOpDesc_t *pOp, const uint8_t *pRspBuf);
...
void lctrAdvRptEnable(lctrAdvRptFilt_t *pAdvFilt, bool filtEna);
...
bool lctrAdvRptCheckDuplicate(lctrAdvRptFilt_t *pAdvFilt, uint64_t hash);
