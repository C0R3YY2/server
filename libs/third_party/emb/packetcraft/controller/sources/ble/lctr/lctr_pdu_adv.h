...
typedef struct
{
  ...
  bool              txAddrRnd;          /*!< Tx address type is random. */
  bool              rxAddrRnd;          /*!< Rx address type is random. */
...
uint8_t lctrUnpackAuxConnRspPdu(lctrAuxConnRsp_t *pPdu, const uint8_t *pBuf);

/* Validate */
bool lctrValidateConnIndPdu(lctrConnInd_t *pPdu);
