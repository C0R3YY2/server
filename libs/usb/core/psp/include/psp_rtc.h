...
{                  /* local time */
  ...
  uint8_t   wday;  /* Days since Sunday (range: 0..6). Not used by filesystems (except exFAT). */
  int8_t    zhour; /* Zone differential time (hours) to UTC (range: -11..11). Not used by filesystems (except exFAT). */
  uint8_t   zmin;  /* Zone differential time (minutes) to UTC (range: 0..59). Not used by filesystems (except exFAT). */
} t_psp_timedate;
